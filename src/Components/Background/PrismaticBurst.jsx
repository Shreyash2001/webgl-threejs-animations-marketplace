...
uniform int uMaxIterations; // New uniform for controlling loop iterations
float hash21(vec2 p){
    p = floor(p);
...
float layeredNoise(vec2 fragPx){
    ...
    // Reduced noise layers for optimization (removed last two)
    return n;
}
...
void main(){
    ...
    for (int i = 0; i < 64; ++i) { // Upper limit for GLSL loop, but controlled by uMaxIterations
...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    // Detect mobile and adjust DPR and max iterations accordingly
    const mobile = isMobile();
    const dpr = mobile ? 1 : Math.min(window.devicePixelRatio || 1, 2);
    const maxIterations = mobile ? 20 : 44; // Reduce iterations on mobile for performance
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uRayCount: { value: 0 },
        uMaxIterations: { value: maxIterations }, // New uniform
      },
    });
    ...
    // Use 'pointermove' which works on both desktop and mobile (touch)
